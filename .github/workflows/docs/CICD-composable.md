...
**Private source checkouts.** The source checkout uses the calling repo's `GITHUB_TOKEN`, which reaches only that repo. If your submodules or sibling repos are separate private repositories, supply `gh-source-app-client-id` plus the `gh-source-app-private-key` secret, or a pre-minted `gh-source-token` secret. Both `reusable_execute-build.yaml` and `reusable_artifacts-cicd.yaml` accept them. Name the repositories in `gh-source-app-repositories`: leaving it empty gives the minted token every repository in the App installation. See the [execute-build README](https://github.com/aerospike/shared-workflows/blob/main/.github/workflows/execute-build/README.md#private-source-checkouts).
