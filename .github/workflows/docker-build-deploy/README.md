...
- `jf-registry` (string, optional): Full Docker registry URL including repository path (e.g., `artifact.aerospike.io/database-container-dev-local`)
...
- `repo-scope` (string, default `-container-dev-local`): Repository suffix appended to jf-project (e.g., `-container-preview-public-local`)
...
1. **Immutable tag** (always generated): `{registry}/{image-name}:{app-version}_{timestamp}` (e.g., `artifact.aerospike.io/database-container-dev-local/test-image:7.0.0_20251202T021054Z`)
2. **Default tags** (when `versions-override` is not provided):
   - Version tag: `{registry}/{image-name}:{app-version}` (e.g., `artifact.aerospike.io/database-container-dev-local/test-image:7.0.0`)
   - Major version tag: `{registry}/{image-name}:{major}` (e.g., `artifact.aerospike.io/database-container-dev-local/test-image:7`)
   - Latest tag: `{registry}/{image-name}:latest` (e.g., `artifact.aerospike.io/database-container-dev-local/test-image:latest`)
...
- For Artifactory, the registry path includes the repository (e.g., `artifact.aerospike.io/database-container-dev-local`)
